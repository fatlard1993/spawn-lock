#!/usr/bin/env python3
"""Generate Spawn Lock's mod menu icon: a cell.

Iron bars over a dark stone wall is the mod in one look, whichever door you met
it at: the pen a newcomer waits in, and the jail an operator puts you in. Source pixels are read
straight out of the vanilla Minecraft jar and scaled nearest neighbour, never
smoothed.
...
OUT = os.path.join(HERE, "src/main/resources/assets/spawn-lock-justfatlard/icon.png")
...
# Where the torch lives inside block/torch.png, and the point it pivots around:
# the middle of its cut end, which is what rests against whatever holds it.
def build_icon():
    """Stone bricks for the wall, darkened so the bars read against them; the
    bars are vanilla's own, laid over the whole face."""
    icon = [[shade(px, 0.45) for px in row] for row in vanilla("block/stone_bricks.png")]
    bars = vanilla("block/iron_bars.png")
    for y in range(16):
        for x in range(16):
            px = bars[y][x]
            if px[3]:
